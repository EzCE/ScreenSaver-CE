...
static void strands_init(const uint8_t numTurtles, Turtle turtles[numTurtles]) {
...
static void strands_walk(const uint8_t numTurtles, Turtle turtles[numTurtles]) {
    for (uint8_t i = 0; i < numTurtles; i++) {
        Turtle* t = &turtles[i];
...
bool strands(void) {
    ...
    strands_init(NUM_TURTLES, turtles);
    gfx_SetDrawScreen();
    ...
    while (!kb_AnyKey()) {
        ...
        strands_walk(NUM_TURTLES, turtles);
